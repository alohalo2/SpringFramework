...
//    }
//
//    @RequestMapping(value = "/name-tel.do", method = RequestMethod.POST) /* ==> @GetMapping("/name-tel.do)*/
...
//                             @RequestParam(value = "tel", required = false, defaultValue = "test") String g,
//                             @RequestParam(value = "tel", required = false, defaultValue = "test") String y,
//                             @RequestParam(value = "tel", required = false, defaultValue = "test") String e,
//                             @RequestParam(value = "tel", required = false, defaultValue = "test") String w,
...
//    }
    ...
    public String nameTelPost(@ModelAttribute NameTelDto nameTelDto, Model model, HttpServletRequest request
        ...
        // 화면단에서는 el표기법(${})으로 키 값에 해당하는 밸류를 사용할 수 있다.
...
//        model.addAttribute("nameTelList", nameTelService.getNameTelList());
        ...
        // HttpServletRequest 객체의 setAttribute(키, 밸류);
//        request.setAttribute("nameTelList", nameTelService.getNameTelList());

        // 2-3. HttpSession 객체를 이용한 데이터 전송
        // Model 객체와 HttpServletRequest 객체는 하나의 요청이 시작된 때 생성돼서 요청이 종료되면 사라지는 객체
...
//        session.setAttribute("nameTelList", nameTelService.getNameTelList());
    ...
    @GetMapping("/name-tel-List.do")
    public String nameTelList() {
        System.out.println("name");
        System.out.println("tel");
        nameTelService.getNameTelList();
        return "name-tel";
    }
